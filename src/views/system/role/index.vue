          ...
          <a-row :gutter="48">
            ...
            <a-col :md="8" :sm="24">
              <a-form-item label="权限字符">
                <a-input v-model="queryParam.roleKey" placeholder="请输入"
                         @keyup.enter="handleQuery" allow-clear/>
              </a-form-item>
            </a-col>
    ...
    <a-card :bordered="false" class="table-card">
      ...
      <a-table
        ...
        <span slot="status" slot-scope="text, record">
          ...
          >
            <span slot="title">确认<b>{{
                record.status === '1' ? '启用' : '停用'
              }}</b>{{ record.roleName }}的角色吗?</span>
            <a-switch default-checked :checked="record.status == 0"/>
        ...
        <span slot="operation" slot-scope="text, record"
              v-if="record.roleId !== 1 && ($store.state.user.parentUnionId===undefined||$store.state.user.parentUnionId===null)">
            ...
            <a-button type="link" icon="user-add" @click="handleAuthUser(record)"
                      v-hasPermi="['system:role:edit']">
              分配用户
            </a-button>
        </span>
  ...
  methods: {
    ...
    handleExport() {
      ...
      this.$confirm({
        ...
        onOk() {
          that.download('/admin/system/role/export', {
            ...that.queryParam
