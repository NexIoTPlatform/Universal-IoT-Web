        ...
        <div class="custom-tabs-container">
          ...
          <div class="custom-tab-content">
              ...
              <div class="device-basic-info">
                ...
                <div class="basic-info-grid">
                  ...
                  <div class="info-item" v-if="deviceNode === 'GATEWAY_SUB_DEVICE'">
                    ...
                    <span class="info-value">{{ devDetails.parentName }}</span>
                  </div>
                  <div class="info-item ">
                ...
                <div class="info-item info-item-full" v-if="devDetails.devGroupName.length>0">
                  <span class="info-label">设备群组</span>
                  <div class="tag-group">
            ...
            <!-- <div class="info-message">
              <span>在线模拟功能可以帮助您测试设备的各种功能，无需实际硬件设备</span>
            </div> -->

            <!-- 设备调试弹窗 -->
            <!-- <device-debugging
              v-if="devDetails.iotId && productDetails.id"
              :meta-data="metaData"
              :device="devDetails"
              :device-node="deviceNode"
              :show="showDeviceDebugging"
              :type="functionType"
              :product-details="productDetails"
              @close="closeDeviceDebugging"
            /> -->
            <!-- </div> -->
            ...
            <div v-show="activeTab === '9'" class="tab-pane" v-if="deviceNode === 'GATEWAY'">
              ...
              <ChildDevice
                ...
                :dev-id="devDetails.deviceId"
                :product-key="devDetails.productKey"
              />
            </div>

            <!-- SIM卡信息 -->
            <!-- <div v-show="activeTab === '10'" class="tab-pane" v-if="iccid">
              <div class="simple-header">
                <h3>SIM卡信息</h3>
                <p>查看和管理设备SIM卡信息，监控网络连接状态</p>
              </div>
              <SIMCard v-if="iccid" :iccid="iccid" />
            </div> -->
          </div>
        </div>
      </a-spin>
...
import MetaLogV2 from './metaLogV2'
  ...
  methods: {
    ...
    async getDevDeatils() {
      ...
      try {
        ...
        // 处理时间格式
        if (this.devDetails.createTime) {
          this.devDetails.createTime = this.formatDate(
            new Date(Number.parseInt(this.devDetails.createTime * 1000)))
        }
        if (this.devDetails.onlineTime) {
          this.devDetails.onlineTime = this.formatDate(
            new Date(Number.parseInt(this.devDetails.onlineTime * 1000)))
        }
        if (this.devDetails.registryTime) {
          this.devDetails.registryTime = this.formatDate(
            new Date(Number.parseInt(this.devDetails.registryTime * 1000)))
        }
        ...
        if (this.devDetails.iotId) {
          getDeviceShadow(this.devDetails.iotId).then((response) => {
            if (response && response.data) {
              this.deviceShadow.lastTime = this.formatDate(new Date(response.data.lastTime))
      ...
      getEventTotal(productKey, iotId).then((res) => {
        if (res && res.rows) {
          res.rows.forEach((item) => {
            if (item.time) {
              item.lastTime = this.formatDate(new Date(Number.parseInt(item.time) * 1000))
            } else {
              item.lastTime = '无'
            }
          })
      ...
      devListShadow(iotId).then((res) => {
        if (res && res.data) {
          res.data.forEach((item) => {
            ...
            if (item.timestamp) {
              item.lastTime = this.formatDate(new Date(item.timestamp * 1000))
            } else {
              item.lastTime = '无'
            }
          })
