  ...
  <div class="main user-layout-register">
    ...
    <a-form-model ref="form" :model="form" :rules="rules">
      ...
      <a-form-model-item has-feedback prop="password">
        <a-input-password v-model="form.password" size="large" autocomplete="off" placeholder="密码"
                          :maxLength="20"/>
      </a-form-model-item>
      <a-form-model-item has-feedback prop="confirmPassword">
        <a-input-password v-model="form.confirmPassword" size="large" autocomplete="off"
...
<script>
import {getCaptchaCode, register} from '@/api/login'
...
export default {
  ...
  data() {
    const validateNewPass = (rule, value, callback) => {
      ...
      } else if (!/^(?![\d]+$)(?![a-zA-Z]+$)(?![^\da-zA-Z]+$)([^\u4e00-\u9fa5\s]){5,20}$/.test(
        value)) {
        callback(
          new Error('请输入5-20位英文字母、数字或者符号（除空格），且字母、数字和标点符号至少包含两种'))
      } else {
