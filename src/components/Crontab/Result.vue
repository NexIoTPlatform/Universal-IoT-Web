<template>
  <a-list size="small" :loading="loading" :data-source="resultList">
...
export default {
  ...
  methods: {
      ...
      goYear: for (let Yi = YIdx; Yi < YDate.length; Yi++) {
        ...
        goMonth: for (let Mi = MIdx; Mi < MDate.length; Mi++) {
          ...
          goDay: for (let Di = DIdx; Di < DDate.length; Di++) {
            ...
            if (this.checkDate(YY + '-' + MM + '-' + thisDD + ' 00:00:00') !== true && this.dayRule
              !== 'workDay' && this.dayRule !== 'lastWeek' && this.dayRule !== 'lastDay') {
              resetDay()
              continue goMonth
            }
            // 如果日期规则中有值时
            if (this.dayRule === 'lastDay') {
              ...
              // 获取达到条件的日期是星期X
              const thisWeek = this.formatDate(new Date(YY + '-' + MM + '-' + thisDD + ' 00:00:00'),
                'week')
              // 当星期日时
              if (thisWeek === 1) {
                // 先找下一个日，并判断是否为月底
              ...
              // 如果指定了是星期几
              // 获取当前日期是属于星期几
              const thisWeek = this.formatDate(new Date(YY + '-' + MM + '-' + DD + ' 00:00:00'),
                'week')
    ...
    formatDate(value, type) {
      ...
      // 如果传递了type的话
      if (type === undefined) {
        return Y + '-' + (M < 10 ? '0' + M : M) + '-' + (D < 10 ? '0' + D : D) + ' ' + (h < 10 ? '0'
