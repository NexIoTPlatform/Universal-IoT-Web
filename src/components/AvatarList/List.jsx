...
const AvatarListProps = {
  ...
  size: {
    validator: val => {
      return typeof val === 'number' || ['small', 'large', 'default'].includes(
        val)
    },
    default: 'default'
  },
...
const AvatarList = {
  ...
  render(h) {
    ...
    const itemsDom = items && items.length ? <ul
      class={`${prefixCls}-items`}>{this.getItems(items)}</ul> : null
  ...
  methods: {
    getItems(items) {
      ...
      if (this.maxLength > 0) {
        ...
        items.push((
          <Avatar size={this.size} style={this.excessItemsStyle}>{`+${totalSize
          - this.maxLength}`}</Avatar>))
      }
