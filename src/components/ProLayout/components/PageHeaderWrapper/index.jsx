...
const PageHeaderWrapper = {
  ...
  render(h) {
    ...
    if (propsBreadcrumb === true) {
      let routes = $route.matched.concat().filter(
        route => route && route.meta && route.meta.title)
      ...
      const defaultItemRender = ({route, params, routes, paths, h}) => {
        return (route.redirect === 'noRedirect' || routes.indexOf(route)
          === routes.length - 1) && (
    ...
    return (
      <div class="ant-pro-page-header-wrap">
        <div class={`${prefixedClassName}-page-header-warp`}>
          <GridContent>{defaultPageHeaderRender(h, props, pageMeta,
            i18n)}</GridContent>
        </div>
        {children ? (
