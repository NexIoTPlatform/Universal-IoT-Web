...
const renderMenu = (h, item, i18nRender, collapsed = false) => {
  if (item && !item.hidden) {
    const bool = item.children && !item.hideChildrenInMenu
    return bool ? renderSubMenu(h, item, i18nRender, collapsed)
      : renderMenuItem(h, item, i18nRender)
  }
  return null
}

const renderSubMenu = (h, item, i18nRender, collapsed) => {
  if (collapsed) {
    return (
      <SubMenu key={item.name} title={renderIcon(h, item.meta.icon)}>
        {!item.hideChildrenInMenu && item.children.map(
          cd => renderMenu(h, cd, i18nRender))}
      </SubMenu>
    )
  }
  return (
...
const RouteMenu = {
  ...
  render(h) {
    const {mode, theme, menus, i18nRender, collapsed} = this
    ...
    const menuItems = menus.map(item => {
      ...
      return renderMenu(h, item, i18nRender, collapsed)
    })
  ...
  methods: {
    updateMenu() {
      ...
      const {hidden, selected, open} = this.$route.meta
      ...
      if (selected) {
        this.selectedKeys = this.selectedKeys.concat(selected)
      }
      if (open) {
        openKeys = openKeys.concat(open)
      }

      this.collapsed ? (this.cachedOpenKeys = openKeys)
        : (this.openKeys = openKeys)
    }
  },
  computed: {
